...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            data = {
                "success": False,
                "message": "Bu email orqali avval ro'yxatdan o'tilgan"
            }
            raise ValidationError(data)
        elif value and User.objects.filter(phone_number=value).exists():
            data = {
                "success": False,
                "message": "Bu telefon raqami orqali avval ro'yxatdan o'tilgan"
            }
            raise ValidationError(data)
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_username(self, username):
        if len(username) < 5 or len(username) > 30:
            raise ValidationError(
                {
                    "message": "Username must be between 5 and 30 charecters long"
                }
            )
        if username.isdigit():
            raise ValidationError(
                {
                    "message": "Bu username faqat sonlardan iborat."
                }
            )
        return username
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == "username":
            ...
            data = {
                "success": True,
                "message": "Siz email, telefon raqam, yoki username kiritishingiz kerak"
            }
            raise ValidationError(data)
        ...
        if user is not None:
            ...
            raise ValidationError(
                {
                    "success": False,
                    "message": "Sorry, your login or password is incorrect. Please check and try again."
                }
            )
